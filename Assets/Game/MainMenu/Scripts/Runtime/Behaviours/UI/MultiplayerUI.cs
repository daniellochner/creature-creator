    ...
    public class MultiplayerUI : MonoBehaviour
    {
        ...
        public bool IsConnectedToInternet
        {
            get
            {
                ...
                {
                    UpdateNetworkStatus(LocalizeUtility.Localize("network_status_internet"), Color.white);
                    return false;
                }
                return true;
            }
        ...
        public bool IsValidPlayer
        {
            get
            {
                ...
                {
                    UpdateNetworkStatus(LocalizeUtility.Localize("network_status_username"), Color.white);
                    return false;
                }
                if (filter.ContainsProfanity(username))
                {
                    UpdateNetworkStatus(LocalizeUtility.Localize("network_status_profanity"), Color.white);
                    return false;
                }
                if (username.Length > 16)
                {
                    UpdateNetworkStatus(LocalizeUtility.Localize("network_status_username-length"), Color.white);
                    return false;
                }
                return true;
            }
        ...
        public bool IsValidWorldName
        {
            get
            {
                ...
                {
                    UpdateNetworkStatus(LocalizeUtility.Localize("network_status_world-name"), Color.white);
                    return false;
                }
                if (worldName.Length > 32)
                {
                    UpdateNetworkStatus(LocalizeUtility.Localize("network_status_world-name-length"), Color.white);
                    return false;
                }
                if (filter.ContainsProfanity(worldName))
                {
                    UpdateNetworkStatus(LocalizeUtility.Localize("network_status_world-name-profanity"), Color.white);
                    return false;
                }
                return true;
            }
        ...
        {
            UpdateNetworkStatus(LocalizeUtility.Localize("network_status_connection-failed"), Color.red);
            IsConnecting = false;
        }
        private void OnClientConnect(ulong clientID)
        {
            UpdateNetworkStatus(LocalizeUtility.Localize("network_status_connected"), Color.green);
        }
        ...
        public async void Join(string id)
        {
            ...
            try
            {
                ...
                {
                    password = await InputDialog.InputAsync(LocalizeUtility.Localize("mainmenu_multiplayer_password_title"), LocalizeUtility.Localize("mainmenu_multiplayer_password_placeholder"), error: LocalizeUtility.Localize("network_status_no-password-provided"));
                    bool isValidPasswordHash = string.IsNullOrEmpty(passwordHash) || sha256.VerifyHash(password, passwordHash);
                    if (!isValidPasswordHash)
                    {
                        throw new Exception(LocalizeUtility.Localize("network_status_invalid-password"));
                    }
                }
                ...
                {
                    throw new Exception(LocalizeUtility.Localize("network_status_incorrect-version", Application.version, version));
                }
                ...
                // Join Lobby
                UpdateNetworkStatus(LocalizeUtility.Localize("network_status_joining-lobby"), Color.yellow, -1);
                ...
                // Join Relay
                UpdateNetworkStatus(LocalizeUtility.Localize("network_status_joining-via-relay"), Color.yellow, -1);
                ...
                // Start Client
                UpdateNetworkStatus(LocalizeUtility.Localize("network_status_starting-client"), Color.yellow, -1);
                Play();
            ...
            catch (Exception e)
            {
                ...
                {
                    UpdateNetworkStatus(LocalizeUtility.Localize("network_status_lobby-error"), Color.red); // TODO: Bug with Lobby returning NullReferenceException?
                }
                else
                {
        ...
        public async void Create()
        {
            ...
            try
            {
                ...
                // Allocate Relay
                UpdateNetworkStatus(LocalizeUtility.Localize("network_status_allocating-relay"), Color.yellow, -1);
                ...
                // Generate Join Code
                UpdateNetworkStatus(LocalizeUtility.Localize("network_status_generating-join-code"), Color.yellow, -1);
                string joinCode = await Relay.Instance.GetJoinCodeAsync(allocation.AllocationId);

                // Create Lobby
                UpdateNetworkStatus(LocalizeUtility.Localize("network_status_creating-lobby"), Color.yellow, -1);
                ...
                // Start Host
                UpdateNetworkStatus(LocalizeUtility.Localize("network_status_starting-host"), Color.yellow, -1);
                Play();
        ...
        public void Filter()
        {
            InputDialog.Input(LocalizeUtility.Localize("mainmenu_multiplayer_filter_title"), LocalizeUtility.Localize("mainmenu_multiplayer_filter_placeholder"), onSubmit: delegate (string filterText)
            {
        ...
        private async Task Authenticate()
        {
            ...
            {
                UpdateNetworkStatus(LocalizeUtility.Localize("network_status_authenticating"), Color.yellow, -1);
